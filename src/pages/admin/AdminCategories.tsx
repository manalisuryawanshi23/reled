...
export function AdminSubcategories() {
  ...
  const [formData, setFormData] = useState({ name: '', slug: '', category_id: '', description: '', sort_order: 0 });
  ...
  const fetchData = async () => {
    ...
    const [subRes, catRes] = await Promise.all([
      supabase.from('subcategories').select('*, category:categories(id, name)').order('sort_order'),
      supabase.from('categories').select('*').order('sort_order'),
    ]);
    if (subRes.data) setSubcategories(subRes.data);
    if (catRes.data) setCategories(catRes.data);
  ...
  const openModal = (sub?: Subcategory) => {
    if (sub) {
      ...
      setFormData({ name: sub.name, slug: sub.slug, category_id: sub.category_id, description: sub.description || '', sort_order: sub.sort_order });
    } else {
      setEditingSubcategory(null);
      setFormData({ name: '', slug: '', category_id: '', description: '', sort_order: 0 });
    }
    setShowModal(true);
  };
    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-dark-100">
            {loading ? (
              <tr><td colSpan={4} className="px-6 py-8 text-center"><span className="loader" /></td></tr>
            ) : subcategories.length === 0 ? (
              <tr><td colSpan={4} className="px-6 py-8 text-center text-dark-400">No subcategories found</td></tr>
            ) : (
